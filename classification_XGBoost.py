...
# Defining the hyperparameters with additional regularization
xgb_params = {
    ...
    'num_class': len(np.unique(train_labels)),  # Set number of classes
...
# Fit the classifier to the training data and capture evaluation metrics
eval_set = [(train_features_final, train_labels), (test_features_final, test_labels)]
xgb_classifier.fit(train_features_final, train_labels, eval_set=eval_set, verbose=True)

# Predict classes on the test data
predictions = xgb_classifier.predict(test_features_final)
...
# Calculate precision, recall, F1-score, and MF1-score
...
plt.savefig("ConfusionMatrix_AllFeatures_XGBoost.png")
plt.show()
