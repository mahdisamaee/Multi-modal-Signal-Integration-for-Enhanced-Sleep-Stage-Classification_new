...
train_labels_main = np.load("concatenated_train_labels_1_truncated_label5to4.npy")
train_data_main = np.load("concatenated_EEG_F3_A2_train_data_STFT_K4.npy")

test_data_main = np.load("concatenated_EEG_F3_A2_val_data_STFT_K4.npy")
test_labels_main = np.load("concatenated_val_labels_1_truncated_label5to4.npy")
################
...
train_data_EEG_Pz_Oz, test_data_EEG_Pz_Oz, train_labels_EEG_Pz_Oz, test_labels_EEG_Pz_Oz = train_test_split(train_data_main, train_labels_main, test_size=0.1, random_state=42, shuffle=True)
...
for epoch in range(epochs):
    ...
    pred_labels = []
    with torch.no_grad():
        for images, labels in test_loader_EEG_Pz_Oz:
            images, labels = images.to(device), labels.to(device)
            features = model2(images)
            outputs = classifier2(features)
            loss = criterion2(outputs, labels.long())  # Convert labels to LongTensor
    ...
    val_loss /= len(test_loader_EEG_Pz_Oz)
...
torch.save(model2.state_dict(), "model2_CNN_2D_STFT_EEG_F3_A2_K4.pth")
torch.save(classifier2.state_dict(), "classifier2_CNN_2D_STFT_EEG_F3_A2_K4.pth")


print("saving features ...")
np.save("EEG_F3_A2_STFT_trainFeatures_K4_CNN2D.npy", features_EEG_Pz_Oz_final)
np.save("EEG_F3_A2_STFT_testFeatures_K4_CNN2D.npy",features_test_EEG_Pz_Oz_final)