...
destination_dir = "ROC_A1_STFT_features_K1"

# checking existance of the destination directory 
os.makedirs(destination_dir, exist_ok=True)

# Defining the window size and overlap
window_size = int(2 * 100)  # 2 seconds of signal at 100 samples per second
...
for filename in os.listdir(source_dir):
    if filename.endswith(".npy") and not filename.endswith("_label.npy"):
        ...
        stft_matrix = np.zeros((data.shape[0], 101, 51), dtype=np.complex64)
