...
source_dir = "data_F3_A2"
destination_dir = "EEG_F3_A2_STFT_features_K2"

# checking existance of the destination directory 
os.makedirs(destination_dir, exist_ok=True)

# Defining the window size and overlap
window_size = int(5 * 100)  # 5 seconds of signal at 100 samples per second
...
for filename in os.listdir(source_dir):
    if filename.endswith(".npy") and not filename.endswith("_label.npy"):
        ...
        stft_matrix = np.zeros((data.shape[0], 251, 21), dtype=np.complex64)

        # Computing the STFT and log-power spectrum for each EEG signal
        for i in range(data.shape[0]):
            # Compute the STFT using the defined parameters
