...
train_data_main = np.load("concatenated_ROC_A1_train_data_STFT_K3.npy")

test_data_main = np.load("concatenated_ROC_A1_val_data_STFT_K3.npy")
...
class CNNClassifier2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool1(self.gelu(self.conv1(x)))
        x = self.pool2(self.gelu(self.conv2(x)))
        x = self.flatten(x)
        x = self.fc1(x)
        #x = self.relu3(x)
        #x = self.fc2(x)
...
torch.save(model2.state_dict(), "model2_CNN_2D_STFT_ROC_A1_K3.pth")
torch.save(classifier2.state_dict(), "classifier2_CNN_2D_STFT_ROC_A1_K3.pth")


print("saving features ...")
np.save("ROC_A1_STFT_trainFeatures_K3_CNN2D.npy", features_EEG_Pz_Oz_final)
np.save("ROC_A1_STFT_testFeatures_K3_CNN2D.npy",features_test_EEG_Pz_Oz_final)